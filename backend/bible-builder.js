...
async function build(supplierData, feedData) {
  ...
  for (const sRow of supplierRows) {
    ...
    masterRows.push({
        ...
        Mode_Of_Transport: carrierPoMeta[poNum]?.mode || sRow.Mode_Of_Transport || po?.lineItems?.[0]?.mode || 'Sea',
    ...
    for (const [poNum, skuMap] of Object.entries(carrierAsnIndex)) {
      ...
      for (const [sku, carrierLine] of Object.entries(skuMap)) {
        ...
        masterRows.push({
          ...
          Mode_Of_Transport: carrierPoMeta[poNum]?.mode || poHdr.Mode_Of_Transport || po?.lineItems?.[0]?.mode || 'Sea',
